#!/usr/bin/env python3
"""
Emit a full-pipeline DWN top-level wrapper:
  fixed_dwn_thermometer -> dwn_top (LUT stack) -> fixed_dwn_groupsum

Produces full_pipeline_top.sv in the existing RTL output dir alongside dwn_top.sv.
Also copies thermometer and groupsum component SV files into the dir.

Usage:
    conda run -n plena2 python scripts/emit_full_pipeline_rtl.py --ckpt-name baseline_n6
    # then on beholder0:
    # vivado -mode batch -source synth_dwn.tcl \\
    #   -tclargs <rtl_dir> <results_dir> xcvu9p-flgb2104-2-i full_pipeline_top 4.0
"""
...
def main():
    ...
    print(f"    -tclargs {os.path.abspath(rtl_dir)} <results_dir> xcvu9p-flgb2104-2-i full_pipeline_top 4.0")
