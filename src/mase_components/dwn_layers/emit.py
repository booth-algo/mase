...
def _emit_lut_layer_rom_clocked(rtl_dir, layer_idx, lut_layer):
    """Generate a clocked ROM-based LUT layer .sv with an output FF register stage."""
    indices = lut_layer.get_input_indices()
    contents = lut_layer.get_lut_contents()
    ...
    module_name = f"fixed_dwn_lut_layer_clocked_{layer_idx}"
...
// ROM-based clocked LUT layer {layer_idx}  (auto-generated by dwn.emit)
module {module_name} #(
...
{idx_cases}
    ...
    logic [OUTPUT_SIZE-1:0] comb_out;
    ...
    generate
        ...
        begin : gen_lut
            ...
            fixed_dwn_lut_neuron #(
                ...
                .data_in_0(lut_in), .data_out_0(comb_out[i])
            );
        end
    endgenerate
...
def _emit_pipelined_variant(rtl_dir, top_name, hw_model):
    """Create dwn_top_clocked.sv using ROM-based clocked LUT layer modules."""
    ...
    for i, layer in enumerate(layers):
        _emit_lut_layer_rom_clocked(rtl_dir, i, layer)
    ...
    for i in range(n):
        instances += f"""\
    fixed_dwn_lut_layer_clocked_{i} lut_layers_{i}_inst (
        .clk(clk), .rst(rst),
        .data_in_0(l{i}_in),
...
/* verilator lint_off UNSIGNED */
// ROM-based thermometer encoder (auto-generated by dwn.emit)
module fixed_dwn_thermometer_clocked #(
    parameter NUM_FEATURES   = {num_features},
    parameter FEATURE_WIDTH  = {feature_width},
    parameter NUM_THRESHOLDS = {num_thresholds}
) (
    ...
    logic [NUM_FEATURES*NUM_THRESHOLDS-1:0] comb_out;
...
{thresh_cases}
    ...
    generate
        ...
        begin : gen_feature
            ...
            begin : gen_threshold
                wire [FEATURE_WIDTH-1:0] tval = TH[ti*FEATURE_WIDTH +: FEATURE_WIDTH];
                assign comb_out[fi*NUM_THRESHOLDS + ti] = (feat > tval);
            end
        end
    endgenerate
...
"""
    ...
    with open(out_path, "w") as f:
        f.write(sv)
    with open(os.path.join(rtl_dir, "fixed_dwn_thermometer_clocked.sv"), "w") as f:
        f.write(sv_clocked)
    # TODO: Only one out_path is returned, should be fine but we should probably only emit one variant
    return out_path
...
def _emit_clocked_wrapper(rtl_dir, cfg, feature_width):
    ...
    num_classes = cfg["num_classes"]
    lut_output = cfg["hidden_sizes"][-1]
    num_lut_layers = len(cfg["hidden_sizes"])
    thermo_bits = num_features * num_bits
    score_width = f"$clog2({lut_output}/{num_classes})"
    # 1 FF after thermometer + 1 FF per LUT layer; groupsum output is direct (no output FF)
    latency = 1 + num_lut_layers

    sv = f"""\
// Full DWN inference pipeline - clocked/registered variant.
// Stages: thermometer (comb) -> FF reg -> dwn_top_clocked -> groupsum (comb)
// Auto-generated by dwn.emit
// Config: input_features={num_features}, num_bits={num_bits}, num_classes={num_classes}
// Input : {num_features} features x {feature_width}-bit integers
// Thermo: {thermo_bits} bits  LUT stack: {lut_output} bits  Output: {num_classes} class scores
// Pipeline latency: {latency} cycles ({num_lut_layers} LUT layers + 1 thermo FF stage)
...
module full_pipeline_top_clocked (
    ...
    assign data_out_0 = {{{','.join(f' gs_comb[{i}]' for i in range(num_classes))}}};
...
def emit_dwn_rtl(
    *,
    ...
    # Pipelined variant
    if emit_pipelined:
        _emit_pipelined_variant(rtl_dir, top_name, hw_model)
